...
async function handleProxy(req: NextRequest, method: string, pathSegments: string[]) {
    ...
    try {
        const url = `${apiUrl}/api/v1/identity/${subpath}${searchParams ? `?${searchParams}` : ''}`;
        ...
        if (!res.ok) {
            ...
            console.error(`[Identity Proxy Error] ${subpath} (${res.status}):`, errorText);
        ...
        console.error(`[Identity Proxy Exception] ${subpath}:`, error);
        return NextResponse.json(
            { error: 'Identity service unreachable', details: error?.message },
            { status: 502 }
        );
    }
}
