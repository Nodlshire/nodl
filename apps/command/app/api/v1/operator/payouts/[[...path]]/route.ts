...
async function handleProxy(req: NextRequest, method: string, pathSegments: string[]) {
    ...
    try {
        const url = `${apiUrl}/api/v1/operator/payouts/${subpath}${searchParams ? `?${searchParams}` : ''}`;
        ...
        if (!res.ok) {
            ...
            console.error(`[Payouts Proxy Error] ${subpath} (${res.status}):`, errorText);
        ...
        console.error(`[Payouts Proxy Exception] ${subpath}:`, error);
        return NextResponse.json(
            { error: 'Payouts provider unreachable', details: error?.message },
            { status: 502 }
        );
    }
}
