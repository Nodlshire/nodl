...
async function handleProxy(req: NextRequest, method: string, pathSegments: string[]) {
    ...
    try {
        const url = `${apiUrl}/api/v1/reputation/${subpath}${searchParams ? `?${searchParams}` : ''}`;
        ...
        if (!res.ok) {
            ...
            console.error(`[Reputation Proxy Error] ${subpath} (${res.status}):`, errorText);
        ...
        console.error(`[Reputation Proxy Exception] ${subpath}:`, error);
        return NextResponse.json(
            { error: 'Reputation service unreachable', details: error?.message },
            { status: 502 }
        );
    }
}
