...
async function handleProxy(req: NextRequest, method: string, pathSegments: string[]) {
    ...
    try {
        const url = `${apiUrl}/api/v1/stake/${subpath}${searchParams ? `?${searchParams}` : ''}`;
        ...
        if (!res.ok) {
            ...
            console.error(`[Stake Proxy Error] ${subpath} (${res.status}):`, errorText);
        ...
        console.error(`[Stake Proxy Exception] ${subpath}:`, error);
        return NextResponse.json(
            { error: 'Stake service unreachable', details: error?.message },
            { status: 502 }
        );
    }
}
