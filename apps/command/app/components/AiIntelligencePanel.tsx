...
export default function AiIntelligencePanel() {
  ...
  return (
    <div className="bg-white/[0.03] shadow-[0_4px_25px_rgba(0,0,0,0.4)] border border-wnode-sovereign p-3 sm:p-4 rounded-[5px] flex flex-col gap-1 group truncate transition-all hover:bg-white/[0.06] backdrop-blur-sm h-full">
      <span className="text-[13px] text-white/85 font-normal uppercase tracking-tight font-sans">
        AI - Mesh Maestro
      </span>
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[18px] font-normal tracking-tighter ${getStatusColor()}`}>
              {getStatusText()}
            </span>
          </div>
          <span className="text-[11px] text-white/60 font-normal font-sans tracking-widest mt-0.5 truncate max-w-full">
            Model: {getModelName()}
          </span>
        </div>
