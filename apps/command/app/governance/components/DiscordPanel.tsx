...
export default function DiscordPanel() {
  ...
  return (
    <div  className="bg-white/[0.02] border border-wnode-border-neutral shadow-[0_0_20px_rgba(255,255,255,0.05)] rounded-[5px] p-8 space-y-6 hover:shadow-[0_0_20px_rgba(168,85,247,0.05)] transition-all group h-[280px] flex flex-col">
      <div className="flex items-center justify-between">
        ...
        <span className="px-2 py-0.5 bg-green-500/10 border border-green-500/40 text-green-500 text-[9px] uppercase font-bold rounded">Server Status: —</span>
      </div>

      <div className="flex-1 space-y-3 overflow-hidden">
        {announcements.map((a, i) => (
          <div key={i} className="flex items-center justify-between p-2 bg-white/5 border border-wnode-border-neutral rounded">
            <span className="text-[11px] text-white/80 truncate pr-4">{a.title}</span>
            <span className="text-[9px] text-white/40 whitespace-nowrap">{a.time}</span>
          </div>
        ))}
      </div>
