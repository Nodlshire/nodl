...
export default function FounderSlotsPanel() {
    const [data, setData] = useState<any>(null);

    const fetchFounders = () => {
        fetch("/api/v1/admin/governance/founders")
            .then(res => res.json())
            .then(setData)
            .catch(console.error);
    };
    ...
    return (
        <div className="bg-slate-900 border border-white/5 rounded-2xl p-6">
            <h2 className="text-white text-lg font-medium mb-6">Founder Slots</h2>
            
            <div className="space-y-4">
                {data.founders?.map((p: any) => {
                    const invite = data.invites?.find((i: any) => i.founderSlot === p.slot);
                    ...
                    return (
                        <div key={p.slot} className="flex items-center justify-between bg-slate-800/50 p-4 rounded-xl border border-white/5">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="text-white font-medium">Founder Slot {p.slot}</div>
