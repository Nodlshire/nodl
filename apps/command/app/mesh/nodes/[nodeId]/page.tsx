...
export default function NodeDetailPage() {
    ...
    if (!node) return <div className="p-8 text-white/40 font-medium">Provider node not found in current mesh snapshot.</div>;
        ...
        <div className="p-8 space-y-8 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white/[0.03] border border-wnode-border-neutral p-6 rounded-lg space-y-2">
                    <div className="text-xs font-medium text-white/40 uppercase tracking-wider">Current Load</div>
                ...
                <div className="bg-white/[0.03] border border-wnode-border-neutral p-6 rounded-lg space-y-2">
                    <div className="text-xs font-medium text-white/40 uppercase tracking-wider">Capacity</div>
                    <div className="text-2xl font-bold text-white">{node.capacity || "N/A"}</div>
                    <div className="text-xs text-white/60">Resource Tokens</div>
                </div>
                <div className="bg-white/[0.03] border border-wnode-border-neutral p-6 rounded-lg space-y-2">
                        ...
                        <tbody className="divide-y divide-white/5 bg-black">
                            {tasks.map(task => (
                                <tr key={task.taskId} className="hover:bg-white/[0.02] group">
                                    ...
                                    <td className="px-6 py-4 capitalize text-white/80">{task.status}</td>
                                    <td className="px-6 py-4 text-white/40 text-xs">{new Date(task.createdAt).toLocaleString()}</td>
