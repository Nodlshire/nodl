          ...
          <div className="space-y-6 flex flex-col h-full lg:px-10 py-10 lg:py-0">
            ...
            <div className="space-y-5 flex-1">
              ...
              {[
                ...
                "Run the OS-specific install script (install_windows.ps1, install_macos.sh, or install.sh) as Administrator/root.",
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {[
                        { os: "Linux (amd64)", cmd: `sudo mkdir -p /etc/wnode && echo "${headlessToken}" | sudo tee /etc/wnode/token && curl -fsSL https://raw.githubusercontent.com/wnodeltd/wnode/main/node-operator/installer/linux/install.sh | bash` },
                        { os: "macOS (Apple Silicon)", cmd: `curl -LO https://github.com/wnodeltd/wnode/releases/download/v1.1.0/nodl-core-darwin-arm64.tar.gz && tar -xzf nodl-core-darwin-arm64.tar.gz && sudo ./install_macos.sh && echo "${headlessToken}" | sudo tee /etc/wnode/token && sudo launchctl kickstart -k system/com.nodl.core` },
                        { os: "Windows (PowerShell)", cmd: `Invoke-WebRequest -Uri "https://github.com/wnodeltd/wnode/releases/download/v1.1.0/nodl-core-windows-amd64.zip" -OutFile "operator.zip"; Expand-Archive "operator.zip" -Force; cd operator; .\\install_windows.ps1; Set-Content -Path "$env:ProgramData\\Wnode\\token" -Value "${headlessToken}"; Restart-Service -Name "nodl-core"` }
                      ].map(item => (
