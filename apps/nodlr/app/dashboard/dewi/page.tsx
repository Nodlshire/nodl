...
export default function OperatorDeWiPage() {
    ...
    useEffect(() => {
        // Fetch operator-isolated nodes from nodld via Nodlr API proxy
        fetch('/api/nodes?scope=user')
            .then(res => res.json())
            .then(data => {
                const fetchedStatuses: Record<string, AdapterStatus> = {};
                const nodeList = Array.isArray(data) ? data : data?.nodes ? Object.values(data.nodes) : [];
            ...
            .catch(err => {
                console.error('Failed to fetch operator DeWi telemetry:', err);
                setLoading(false);
            });
    }, []);
    ...
    return (
        <div className="space-y-6 p-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-4">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-2xl font-bold text-white tracking-tight">DeWi Operator Panel</h1>
                        <span className="px-2 py-0.5 text-xs font-semibold rounded bg-[#22D3EE]/10 text-[#22D3EE] border border-[#22D3EE]/30">
                            WUID Operator Scope
                        </span>
                    </div>
                    <p className="text-sm text-slate-400 mt-1">
                        Account-level wireless hardware management, RF region profile compliance, and proof lineage.
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricCard
                    label="My Active Adapters"
                    value={Object.keys(statuses).length.toString()}
                    icon={Radio}
                    subValue="Operator Hardware Scope"
                />
                <MetricCard
                    label="RF Region Profile"
            ...
            {/* Operator Hardware Adapters Grid */}
            <div className="bg-[#0b101d] rounded-lg border border-white/10 p-5">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                        <Cpu className="w-5 h-5 text-cyan-400" />
                        My Wireless Adapters & Hardware Status
                    </h2>
                    <div className="flex items-center gap-2 text-xs text-slate-400">
                        <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                        Operator Telemetry Normalization
                    </div>
                </div>

                {loading ? (
                    <div className="text-center py-8 text-slate-500 text-xs font-mono">
                        Loading operator hardware telemetry...
                    </div>
                ) : Object.keys(statuses).length === 0 ? (
                    <div className="text-center py-8 text-slate-500 text-xs font-mono">
                        No active wireless adapters registered under your WUID.
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {Object.entries(statuses).map(([proto, st]) => (
                            <div key={proto} className="bg-[#070b14] border border-white/10 rounded-lg p-4 space-y-3 relative overflow-hidden">
            ...
            {/* Region & Operator Safety Gate Control */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Region Compliance Control */}
                <div className="bg-[#0b101d] rounded-lg border border-white/10 p-5 space-y-4">
                    <h3 className="text-md font-semibold text-white flex items-center gap-2">
                        <ShieldCheck className="w-5 h-5 text-emerald-400" />
                        Frequency & Region Compliance Enforcer (FRCL)
                    </h3>
                    <p className="text-xs text-slate-400">
                        Select regulatory RF region for your hardware. Duty-cycle caps are enforced before adapters enter Ready state.
                    </p>
                ...
                <div className="bg-[#0b101d] rounded-lg border border-white/10 p-5 space-y-4">
                    ...
                    <p className="text-xs text-slate-400">
                        Transmission is disabled by default. Enabling TX for your hardware requires a signed operator token.
                    </p>
                    ...
                    <div className="flex items-center gap-2 pt-1">
                        ...
                        <span className="text-xs text-slate-500">Status: RX-Only Safe</span>
                    </div>
                </div>
            </div>

            {/* Operator Proof Lineage Chain Inspector */}
            <div className="bg-[#0b101d] rounded-lg border border-white/10 p-5 space-y-4">
                <div className="flex items-center justify-between">
                    <h3 className="text-md font-semibold text-white flex items-center gap-2">
                        <Layers className="w-5 h-5 text-purple-400" />
                        Operator Cryptographic Proof Lineage Chain (SOT Ledger)
                    </h3>
                            ...
                            {logs.length === 0 ? (
                                <tr>
                                    <td colSpan={7} className="text-center py-6 text-slate-500 text-xs font-mono">
                                        No cryptographic proof lineage logs recorded for your account in SOT ledger.
                                    </td>
                                </tr>
                            ) : (
                                logs.map((log) => (
