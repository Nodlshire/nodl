...
export function middleware(request: NextRequest) {
  ...
  if (isApiPage) {
    ...
    const isPublicApi = request.nextUrl.pathname === '/api/account/me' || request.nextUrl.pathname.startsWith('/api/auth') || request.nextUrl.pathname.startsWith('/api/v1/') || request.nextUrl.pathname.startsWith('/api/download');
