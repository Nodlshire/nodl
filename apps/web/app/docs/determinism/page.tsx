        ...
        <>
            <div className="border-b border-slate-800 pb-8 mb-12">
                ...
                <p className="text-xl text-slate-400 font-light leading-relaxed">
                    Achieving mathematical parity across heterogeneous global infrastructure through strict WASM bounds and pure functional paradigms.
                </p>
            </div>

            <h2 id="conceptual-overview">Conceptual Overview & Rationale</h2>
            <p>
                In a centralized system, non-determinism is an annoyance (e.g., a flaky test). In a decentralized compute mesh relying on Quorum Reduction, non-determinism is a catastrophic failure. If Node A and Node B run the exact same WASM payload but return different output hashes, the Orchestrator cannot achieve quorum.
            </p>
            <p>
                <strong>The Rationale:</strong> Wnode enforces determinism at the compiler level. By utilizing Wazero and stripping out host bindings, we eliminate the primary vectors for non-determinism: system clocks, pseudo-random number generators (PRNGs), network I/O, and concurrent thread racing.
            </p>
            ...
            <div className="my-12 p-8 bg-slate-900/50 rounded-xl border border-slate-800 flex items-center justify-center">
                <svg viewBox="0 0 800 300" className="w-full drop-shadow-2xl font-sans">
                    ...
                    <text x="150" y="80" fill="white" fontSize="14" fontWeight="bold" textAnchor="middle">WASM Sandbox</text>
            ...
            <div className="my-12 p-8 bg-slate-900/50 rounded-xl border border-slate-800">
                <svg viewBox="0 0 800 250" className="w-full drop-shadow-2xl font-sans text-xs">
                    ...
                    <text x="125" y="115" fill="#94a3b8" textAnchor="end">2. Modifies WASM</text>
            ...
            <p>If your logic requires randomness (e.g. shuffling a deck of cards), you cannot use <code>rand()</code>. You must pass a cryptographically secure seed via the payload and use a deterministic PRNG.</p>
            <CodeBlock language="rust" title="src/lib.rs (Seeded PRNG inside WASM)">{`use rand::prelude::*;
...
pub extern "C" fn shuffle(ptr: *const u8, len: usize) -> *mut u8 {
    ...
    // ... return logic
            ...
            <ul>
                <li><strong>Float Drift:</strong> Floating-point arithmetic (<code>f32</code>, <code>f64</code>) can exhibit microscopic drift across different CPU architectures (x86 vs ARM64) due to varying instruction sets. Wnode highly recommends using integer math or fixed-point arithmetic crates in Rust for financial calculations.</li>
            </ul>

            <h2 id="security-boundaries">Security Boundaries & Invariants</h2>
            <p>
                <strong>Invariant:</strong> The WASM sandbox operates in total isolation from the host clock. If an integration needs to know the "current time" to validate an expiration window, the Orchestrator injects an authoritative, synchronized <code>timestamp</code> field into the payload before routing.
            </p>

            <h2 id="performance">Performance Characteristics</h2>
            <p>
                Enforcing determinism imposes zero performance overhead at runtime. Because Wazero natively lacks these bindings, there are no "interceptor" checks slowing down the execution.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-8 not-prose">
                <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl">
                    ...
                    <p className="text-sm text-slate-400">Run any CPU architecture you want. The WASM byte-code compilation ensures your AMD EPYC produces the exact same binary output as an ARM graviton processor.</p>
                </div>
                <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl">
                    <h4 className="text-white font-bold mb-2">Developer</h4>
                    <p className="text-sm text-slate-400">Do not use floats for financial math. Use deterministic seeding for randomness. Pass all required external state into the payload.</p>
                </div>
            </div>

            <h2 id="telemetry">Telemetry Emitted</h2>
            <p>When a non-deterministic result is caught by the Orchestrator:</p>
  ...
  "action": "slash_deviants"
            ...
            <Callout type="warning" title="Anti-Pattern: Timestamps inside WASM">
                Never attempt to compile a Rust crate like <code>chrono</code> that relies on system time. The compilation to <code>wasm32-unknown-unknown</code> will succeed, but it will panic immediately when it tries to hit the stubbed Wazero syscall.
            </Callout>
