...
export default function DeveloperGuide() {
    return (
        <>
            ...
            <p>
                <strong>The Rationale:</strong> Wnode forces you to program declaratively. You define the shape of your compute (the <code>spec.yaml</code>), and you provide a pure, side-effect-free WASM binary to execute the math. The Orchestrator handles the imperative retries, network boundaries, and consensus.
            </p>
            ...
            <div className="my-12 p-8 bg-slate-900/50 rounded-xl border border-slate-800 flex items-center justify-center">
                <svg viewBox="0 0 800 350" className="w-full drop-shadow-2xl font-sans">
                    ...
                    <text x="150" y="185" fill="white" fontSize="12" textAnchor="middle">2. Code Rust (WASM)</text>
            ...
            <div className="my-12 p-8 bg-slate-900/50 rounded-xl border border-slate-800">
                <svg viewBox="0 0 800 200" className="w-full drop-shadow-2xl font-sans text-xs">
                    ...
                    <text x="400" y="105" fill="#cbd5e1" textAnchor="middle">2. cargo build --target wasm32-unknown-unknown</text>

                    <line x1="600" y1="150" x2="210" y2="150" stroke="#10b981" strokeWidth="2" markerEnd="url(#seqArrowDev)" />
                    <text x="400" y="145" fill="#10b981" textAnchor="middle">3. validate.wasm generated (850kb)</text>
                </svg>
            </div>

            <h2 id="real-code-examples">The Pure Function Paradigm</h2>
            <CodeBlock language="rust" title="src/lib.rs (No Side Effects)">{`// This function is "Pure". It takes an input, does math, and returns output.
...
pub extern "C" fn calculate_shard(ptr: *const u8, len: usize) -> *mut u8 {
    ...
    // BAD: reqwest::get("https://api.price.com") // Will instantly panic in Wazero
    ...
    res_ptr
            ...
            <ul>
                <li><strong>Network Bound Failures:</strong> Attempting to import networking libraries into your Rust WASM target will either fail to compile, or compile but instantly panic at runtime because Wazero stubs all host-network bindings to throw <code>ErrNoSys</code>. </li>
                <li><strong>Non-Deterministic WASM:</strong> If your WASM binary relies on a random number generator (RNG) and it executes on the Space Mesh, Shard 1 on Node A will return a different hash than Shard 1 on Node B. The Orchestrator's Quorum anchor will reject both results as a mismatch. WASM logic MUST be 100% deterministic.</li>
            </ul>

            <h2 id="security-boundaries">Security Boundaries & Invariants</h2>
            <p>
                <strong>Invariant:</strong> Data provided to your WASM binary is trusted. The Orchestrator has already validated the HMAC signature and the nonce. Do not waste precious WASM compute cycles attempting to re-validate authentication.
            </p>

            <h2 id="performance">Performance Characteristics</h2>
            <p>
                A standard `spec.yaml` node execution should complete in under <code>200ms</code>. The timeout limits are strictly enforced. If you provide an <code>O(n^2)</code> algorithm in your WASM and the payload grows too large, the node will kill the job mid-execution. Optimize your Rust data structures (e.g. use <code>BTreeMap</code> instead of iterating large arrays).
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-8 not-prose">
                <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl">
                    ...
                    <p className="text-sm text-slate-400">(N/A for developers. Operators provide the raw power).</p>
                </div>
                <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl">
                    <h4 className="text-white font-bold mb-2">Developer Responsibilities</h4>
                    <p className="text-sm text-slate-400">Design your data. If you are building a Space Mesh MapReduce job, ensure the payload can be cleanly sharded into chunks that do not rely on each other's state.</p>
                </div>
            </div>

            <h2 id="telemetry">Telemetry Emitted</h2>
            <p>The developer does not emit telemetry directly. The generated Go wrappers catch your Rust panics and emit them automatically:</p>
  ...
  "job_id": "job_9x8c7v"
            ...
            <p>
                The developer interacts almost exclusively with the <code>spec.yaml</code> and the Rust toolchain. The output interacts with the CI Verifier, which then passes it to the Orchestrator, which distributes it to the Operators.
            </p>

            <h2 id="best-practices">Best Practices & Anti-Patterns</h2>
            <Callout type="warning" title="Anti-Pattern: Bloated WASM">
                Do not compile massive binaries. If your `.wasm` file is 30MB, Node Operators will experience massive cold-start latency when downloading your substrate upgrade. Strip symbols and optimize for size.
            </Callout>
            <Callout type="best-practice" title="Best Practice: Pass Data In">
                Since WASM cannot fetch data, design your DApp client (the one triggering the SDK) to fetch all required external state, bundle it into the JSON payload, and send it <em>into</em> the mesh for stateless verification.
            </Callout>
        </>
    );
}
