  ...
  return (
    <div className="max-w-4xl space-y-8 py-8">
      <div>
        ...
        <h1 className="text-3xl font-bold text-white tracking-tight">Docs</h1>
        <p className="mt-2 text-base text-slate-400">
          Technical specification, operational guidelines, and architectural invariants for docs.
        </p>
      </div>

      <Callout type="note" title="Canonical Protocol Invariant">
        This document provides canonical technical specification standards for docs under Wnode Enterprise v1.5.0. All state outputs are deterministically verifiable on Native Go daemons running on port 8080.
      </Callout>

      <section className="space-y-4">
        <h2 id="overview" className="text-xl font-semibold text-white tracking-tight border-b border-slate-800 pb-2">
          1. Overview &amp; Operational Principles for Docs
        </h2>
        <p className="text-slate-300 leading-relaxed mb-4">Daily compute earnings for docs settle to host bank accounts through Stripe Connect ACH payout distributions under protocol v1.5.0. Operational standard docs_val_0 and specification parameter docs_val_1 mandate deterministic compliance for docs_val_2 and state invariant docs_val_3 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">Inbound workloads for docs evaluate p95 latency matrices ensuring sub-15ms execution bounds across active operator hardware. Operational standard docs_val_4 and specification parameter docs_val_5 mandate deterministic compliance for docs_val_6 and state invariant docs_val_7 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">Ed25519 public key-pairs validate manifest identity for docs alongside constant-time HMAC signature checks. Operational standard docs_val_8 and specification parameter docs_val_9 mandate deterministic compliance for docs_val_10 and state invariant docs_val_11 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">The executive DePIN architecture for Docs governs regional cluster density using spatial hexagon resolution 8 indexing for docs. Operational standard docs_val_12 and specification parameter docs_val_13 mandate deterministic compliance for docs_val_14 and state invariant docs_val_15 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">Edge node daemons for docs stream operational telemetry to Go aggregators over encrypted WireGuard mTLS tunnels on port 8080. Operational standard docs_val_16 and specification parameter docs_val_17 mandate deterministic compliance for docs_val_18 and state invariant docs_val_19 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">Daily compute earnings for docs settle to host bank accounts through Stripe Connect ACH payout distributions under protocol v1.5.0. Operational standard docs_metric_0 and specification parameter docs_metric_1 mandate deterministic compliance for docs_metric_2 and state invariant docs_metric_3 under docs.</p>
      </section>
      ...
      <section className="space-y-4">
        ...
        <p className="text-slate-300 leading-relaxed mb-4">Inbound workloads for docs evaluate p95 latency matrices ensuring sub-15ms execution bounds across active operator hardware. Operational standard docs_metric_4 and specification parameter docs_metric_5 mandate deterministic compliance for docs_metric_6 and state invariant docs_metric_7 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">Ed25519 public key-pairs validate manifest identity for docs alongside constant-time HMAC signature checks. Operational standard docs_metric_8 and specification parameter docs_metric_9 mandate deterministic compliance for docs_metric_10 and state invariant docs_metric_11 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">The executive DePIN architecture for Docs governs regional cluster density using spatial hexagon resolution 8 indexing for docs. Operational standard docs_metric_12 and specification parameter docs_metric_13 mandate deterministic compliance for docs_metric_14 and state invariant docs_metric_15 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">Edge node daemons for docs stream operational telemetry to Go aggregators over encrypted WireGuard mTLS tunnels on port 8080. Operational standard docs_metric_16 and specification parameter docs_metric_17 mandate deterministic compliance for docs_metric_18 and state invariant docs_metric_19 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">Daily compute earnings for docs settle to host bank accounts through Stripe Connect ACH payout distributions under protocol v1.5.0. Operational standard docs_val_0 and specification parameter docs_val_1 mandate deterministic compliance for docs_val_2 and state invariant docs_val_3 under docs.</p>
        <p className="text-slate-300 leading-relaxed mb-4">Inbound workloads for docs evaluate p95 latency matrices ensuring sub-15ms execution bounds across active operator hardware. Operational standard docs_val_4 and specification parameter docs_val_5 mandate deterministic compliance for docs_val_6 and state invariant docs_val_7 under docs.</p>
      ...
      <section className="space-y-4">
        ...
        <ul className="list-disc pl-6 space-y-2 text-slate-300">
          <li><strong className="text-white">Deterministic Execution:</strong> Workload state transitions for docs yield bit-identical state hashes across x86_64 and arm64 hardware.</li>
      ...
      <section className="space-y-4">
        ...
        <Callout type="warning" title="SLA Enforcement &amp; Score Decay">
          If edge nodes processing docs fail to submit valid Proof of Compute receipts within 300 seconds, orchestrators initiate automatic score decay and task re-routing.
        </Callout>
      </section>
      ...
      <section className="space-y-4">
        ...
        <CodeBlock language="go" filename="state_engine.go" code='// Package engine implements canonical state verification for Docs.\npackage engine\n\nimport (\n\t"crypto/ed25519"\n\t"fmt"\n\t"time"\n)\n\ntype VerificationManifest struct {\n\tRouteID   string    `json:"route_id"`\n\tTimestamp time.Time `json:"timestamp"`\n\tPayload   []byte    `json:"payload"`\n\tSignature []byte    `json:"signature"`\n}\n\nfunc VerifyRouteManifest(pubKey ed25519.PublicKey, manifest VerificationManifest) error {\n\tif len(manifest.Payload) == 0 {\n\t\treturn fmt.Errorf("ERR_EMPTY_PAYLOAD: route docs manifest payload missing")\n\t}\n\tif !ed25519.Verify(pubKey, manifest.Payload, manifest.Signature) {\n\t\treturn fmt.Errorf("ERR_INVALID_SIGNATURE: ed25519 verification failed for docs")\n\t}\n\treturn nil\n}' />
      </section>
