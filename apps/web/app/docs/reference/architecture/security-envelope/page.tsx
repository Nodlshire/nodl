...
import rehypeRaw from 'rehype-raw'; // Add rehypeRaw to support <video> tags

export default function SecurityEnvelopePage() {
    const filePath = path.join(process.cwd(), '../../docs/architecture/security-envelope.md');
    ...
    return (
        <div className="prose prose-invert max-w-none">
            <ReactMarkdown
                rehypePlugins={[rehypeRaw as any]} // Enables rendering raw HTML tags like <video>
                components={{
                    ...
                    video: ({node, ...props}) => <video className="rounded-xl border border-slate-800 shadow-2xl my-8 max-w-full" {...props} />,
                }}
            >
                {markdownContent}
            </ReactMarkdown>
