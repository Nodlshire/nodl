...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Appendix: Constraints & Reference`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Comprehensive reference for all system boundaries and limits.</li>
                    <li>Strict adherence to memory and CPU architectural guidelines.</li>
                    <li>Clarifies the technical boundaries that ensure zero-trust security.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">To build integrations and operate nodes on the Sovereign Mesh without causing unhandled faults or receiving slash penalties, developers and operators must understand the absolute physical and software constraints enforced by the network. This appendix consolidates these hard limits.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Developer consults limits → Architect integration within 2MB binary limit → Optimize algorithms for 64-page RAM bounds → Validate zero network I/O compliance → Deploy successfully.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Enforcing the 64-page limit at runtime
config := wazero.NewRuntimeConfig().
  WithMemoryLimitPages(64) // 64 * 64KB = 4MB Hard Cap`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Exceeding memory allocation inside WASM will trigger an unrecoverable `out of memory` WASI trap. Attempting to use host syscalls will result in an immediate `ErrNoSys`. Attempting to float calculations will result in cross-architecture hash drift, failing Quorum.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">All boundaries are enforced synchronously at the edge. The Orchestrator does not filter inputs for size before sending; the edge node immediately crashes the sandbox upon violation and returns the deterministic fault.</p>
  ...
  "event": "sandbox_constraint_violation",
  "limit_type": "memory_pages",
  "limit_value": 64,
  "attempted_value": 65
}`}</code></pre>
                    </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Constraints apply across the entire pipeline. The TinyGo compiler enforces the 2MB size limit during CI; the Wazero daemon enforces the 64-page memory limit during execution; and the Quorum engine enforces determinism during validation.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Always utilize the `wnode-sim` local CLI testing tool with the `--enforce-constraints` flag enabled to verify your integration before pushing to the global pipeline.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: System Architecture Boundaries Diagram ]</span>
            </div>
