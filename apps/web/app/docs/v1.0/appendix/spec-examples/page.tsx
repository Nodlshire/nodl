...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Appendix: Spec & Proposal Support`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Strict adherence to the core WebAssembly 1.0 specification.</li>
                    <li>Experimental WASM proposals (Threads, SIMD) are explicitly disabled.</li>
                    <li>Provides absolute cross-architecture consistency (ARM vs AMD).</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">While modern WASM engines support advanced proposals like SIMD (Single Instruction, Multiple Data) or multi-threading, these features often behave differently depending on the underlying hardware architecture. A SIMD instruction on an ARM64 Apple M3 chip might yield slightly different rounding behavior than on an AMD EPYC server. To guarantee determinism across a heterogeneous mesh, all experimental proposals are stripped.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Developer builds with SIMD flag → TinyGo compiler blocks flag → OR Orchestrator Registry ingests WASM → Validation checks for SIMD opcodes → Orchestrator rejects artifact.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`# Ensure no experimental features are bundled
wazero run \
  -wasm-features= \     # Disables all non-core features (SIMD, Threads, Bulk Memory)
  build/integration.wasm`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If a WASM artifact containing multi-threading proposals somehow bypasses CI and reaches an edge operator, the Wazero runtime instantiation phase will immediately panic with an `invalid opcode` trap. The operator safely returns the failure trap to the Orchestrator, and no execution occurs.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Sovereign Mesh execution environment guarantees single-threaded, synchronous execution bounded strictly by 64 pages of linear memory. This is non-negotiable for deterministic consensus.</p>
  ...
  "event": "wazero_instantiation_failed",
  "reason": "unsupported_wasm_feature",
  "feature": "simd",
  "action": "registry_rejection"
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Integration Registry performs a static opcode scan before publishing any new integration version. It acts as the primary firewall preventing non-compliant feature opcodes from reaching the Mesh Router.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Avoid using third-party cryptographic libraries written in Go unless they explicitly support fallback to standard math. Many crypto libraries automatically inject SIMD or Assembly instructions to speed up calculations, which will fail compilation.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: WASM Feature Flag matrix ]</span>
            </div>
