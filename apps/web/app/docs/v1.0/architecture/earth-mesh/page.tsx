        ...
        <div className="w-full pb-24">
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Synchronous, low-latency execution tier.</li>
                    <li>Utilizes bare-metal and high-performance fiber connections.</li>
                    <li>Strict requirement for 99.99% uptime among Tier-1 operators.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Earth Mesh is engineered specifically for workloads where latency is the primary constraint. Financial execution (e.g., decentralized liquidations, algorithmic trading) cannot tolerate the asynchronous delays inherent to satellite uplinks. Thus, Earth Mesh nodes are physically clustered near major exchange endpoints and interconnected via high-throughput fiber to ensure sub-50ms execution times.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Latency-critical Job Ingress → Shard Geographic Resolution → Synchronous WSS Dispatch to Tier-1 Operators → Wazero Sub-10ms Init → Execution → Immediate Quorum Verification.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Earth Mesh Synchronous Dispatcher
func DispatchEarthMesh(ctx context.Context, job Job) error {
  // Enforce tight timeout for synchronous tasks
  ctx, cancel := context.WithTimeout(ctx, 50*time.Millisecond)
  defer cancel()
  return BroadcastToTier1(ctx, job)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If an Earth Mesh node experiences hardware degradation or network latency exceeding the 50ms window, the Orchestrator will aggressively cull the node from the active Tier-1 routing table and silently shift the load to redundant operators in the same geographic shard.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Earth Mesh operators must strictly utilize persistent WSS connections; polling architectures are fundamentally banned due to latency overheads. Hardware must support sub-10ms Wazero cold starts consistently.</p>
  ...
  "event": "earth_execution",
  "shard": "us-east-1",
  "latency_ms": 14.2,
  "operator_class": "tier-1-baremetal"
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Earth Mesh operators interact with the core Orchestrator exclusively over low-latency WebSockets. Their output hashes must reach consensus immediately to finalize state transitions before the 50ms window expires.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Node operators on the Earth Mesh should allocate dedicated CPU cores (via cgroups) specifically for Wazero threads to prevent OS scheduler jitter from violating the strict 10ms initialization boundary.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Earth Mesh Topology Diagram ]</span>
            </div>
