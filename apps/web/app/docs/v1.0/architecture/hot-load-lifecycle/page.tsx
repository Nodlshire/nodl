...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Architecture: Hot Load Lifecycle`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Dynamic, sub-10ms WASM module instantiation.</li>
                    <li>AOT (Ahead-of-Time) compilation utilized strictly inside Wazero sandbox.</li>
                    <li>In-memory execution exclusively, bypassing standard disk I/O.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Traditional container scaling is vastly too slow for high-frequency determinism. A Docker cold start ranges from hundreds of milliseconds to several seconds. The hot load lifecycle of our WASM runtime drops initialization latency below 10ms, allowing operators to spin up entirely new logic routines instantly upon receiving a payload over WSS.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">WSS Byte Array Ingress → Cryptographic Signature Verification → Wazero Memory Buffer Allocation → AOT Compilation → Instantiation → Execution → Host Memory Teardown.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Hot Load Pipeline
func HotLoad(wasmBytes []byte) error {
  // Directly instantiate from memory slice, bypassing disk
  mod, err := runtime.Instantiate(ctx, wasmBytes)
  if err != nil { return err }
  defer mod.Close(ctx) // Ensures memory is dropped
  return executeModule(mod)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If the provided `wasmBytes` are malformed or exceed the 2MB artifact threshold, instantiation fails instantly. Further, if the module attempts to allocate more than the initial 64-page contiguous memory block during the hot load, Wazero triggers a deliberate OOM trap.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Hot loaded binaries must never touch the physical disk of the operator. They are received in memory, compiled in memory, executed in memory, and immediately wiped via garbage collection.</p>
  ...
  "artifact_hash": "a4d9..."
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The operator node's WSS handler directly streams bytes into the hot-loading runtime. This tightly couples network ingress and execution, removing intermediate queue layers on the edge node entirely.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Operator environments should ensure Go garbage collection tuning (`GOGC`) is optimized to quickly clear discarded module memory, preventing slow memory inflation over millions of hot-loads.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Hot Load Lifecycle Diagram ]</span>
            </div>
