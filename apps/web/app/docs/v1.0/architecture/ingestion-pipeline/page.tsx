...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Architecture: Ingestion Pipeline`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>High-throughput API Gateway for external REST integration.</li>
                    <li>Strict cryptographic ingress checks using Ed25519 signatures.</li>
                    <li>Asynchronous bridging to internal Orchestrator queues.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Sovereign Mesh requires a hardened boundary between external web traffic and internal node infrastructure. The Ingestion Pipeline serves as this DMZ, enforcing strict DDoS mitigation, cryptographic payload authentication, and schema validation before any data is allowed to touch the Orchestrator's internal routing tables.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Client HTTPS POST → Gateway Rate Limiter → Ed25519 Payload Auth → JSON Schema Validation → Enqueue to Orchestrator Core → Return 202 Accepted.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// HTTP Ingress Handler
func HandleIngress(w http.ResponseWriter, r *http.Request) {
  if !VerifyHMAC(r.Header.Get("X-Signature"), r.Body) {
    http.Error(w, "Unauthorized", 401)
    return
  }
  orchestratorQueue.Push(r.Body)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If a client submits a job that fails cryptographic authentication, or exceeds the strict size constraints (e.g. attempting to submit an oversized WASM payload directly rather than a hash), the Gateway terminates the connection instantly, preventing internal bandwidth exhaustion.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">No payload is ever executed synchronously during the ingress phase. The Ingestion Pipeline is strictly non-blocking; it validates and queues, ensuring the public-facing API maintains sub-millisecond response times regardless of backend execution loads.</p>
  ...
  "queue_depth": 142
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Gateway layer interacts only with the Orchestrator via an internal gRPC stream. It has zero knowledge of, or direct connection to, the Earth or Space mesh operators. This structural isolation guarantees security.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">External clients should implement exponential backoff if they receive a 429 Too Many Requests from the Ingestion layer. Retrying immediately will trigger automated WAF blocking.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Ingestion Pipeline Flow Diagram ]</span>
            </div>
