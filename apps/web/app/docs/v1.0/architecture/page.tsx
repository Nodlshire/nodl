...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Architecture: Global Mesh`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Bifurcated routing based on latency constraints.</li>
                    <li>Earth Mesh for synchronous, low-latency execution (Tier-1).</li>
                    <li>Space Mesh for highly resilient, asynchronous batch compute (Tier-3).</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Not all computation requires the same network topology. Real-time trading algorithms demand sub-50ms hops via the Earth Mesh, whereas large-scale satellite MapReduce jobs prioritize censorship resistance over speed via the Space Mesh. The Orchestrator intelligently bifurcates traffic to optimize resource allocation across these disparate physical layers.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">API Gateway → Global Orchestrator → Execution Constraint Evaluation → Route to Earth Mesh (Fiber/Bare Metal) OR Route to Space Mesh (Satellite/LEO) → Operator Execution → Quorum Settlement.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Core Mesh Routing Logic
func RoutePayload(req Payload) error {
  if req.LatencyBudget < 50 * time.Millisecond {
    return earthmesh.Dispatch(req)
  }
  return spacemesh.EnqueueBatch(req)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If an Earth Mesh partition occurs, the Orchestrator will aggressively re-route the workload to an alternate geographic shard. If a Space Mesh transmission drops, the payload remains in the asynchronous queue for automatic satellite re-transmission during the next optimal orbital window.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Regardless of the physical routing layer (Earth or Space), all execution nodes must adhere strictly to the WASM sandbox invariants. [STATUS: Pilot Mode. Full technical implementation details in Phase 4]</p>
  ...
  "event": "mesh_bifurcation",
  "target_layer": "earth_tier_1",
  "payload_hash": "e3b0c44298...",
  "shard_id": "eu-central-alpha"
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The global Orchestrator acts as the sole bridging mechanism between the synchronous Earth Mesh and the asynchronous Space Mesh. Operators on the Earth Mesh cannot directly initiate compute requests on the Space Mesh.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Developers must declare realistic latency budgets in their `spec.yaml`. Setting an unnecessarily aggressive sub-50ms budget for a batch processing task will inefficiently consume high-value Earth Mesh resources.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Global Mesh Bifurcation Diagram ]</span>
            </div>
