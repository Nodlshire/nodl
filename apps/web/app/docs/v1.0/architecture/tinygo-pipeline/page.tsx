        ...
        <div className="w-full pb-24">
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Enforces a strictly deterministic subset of Go.</li>
                    <li>Aggressive dead-code elimination guarantees {"<"}2MB artifacts.</li>
                    <li>No access to 'net', 'os', or 'time' packages.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Standard Go binaries routinely exceed 10MB due to the inclusion of the large standard library and heavy runtime garbage collection components. To achieve sub-10ms initialization limits, we enforce compilation exclusively via TinyGo. TinyGo's LLVM-based dead-code elimination produces extremely lean WASM modules that adhere strictly to our 2MB maximum payload size limit.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Declarative spec.yaml → `generate_all` emits raw Go stubs → TinyGo compiler (`-target=wasi`) → WASM Binary → Optimization pass → Payload Signature.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`# Internal CI Compilation Target
tinygo build -o build/module.wasm -target=wasi -no-debug -panic=trap ./cmd/module`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Attempting to import standard Go packages that rely on non-deterministic system calls (such as `net/http` or `crypto/rand`) will result in an immediate compiler error from TinyGo, preventing non-compliant code from ever reaching the deployment pipeline.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The compilation toolchain is deeply locked. No developer may bypass TinyGo compilation. WASM artifacts compiled via `GOOS=wasip1 GOARCH=wasm go build` are actively rejected by the Orchestrator due to size and signature mismatches.</p>
  ...
  "event": "compiler_pass",
  "result": "success",
  "binary_size_bytes": 105432,
  "dead_code_elimination_ratio": 0.82
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The TinyGo Pipeline lives entirely within the CI environment. It acts as the gatekeeper between human-readable intent (YAML) and machine-executable artifact (WASM), ensuring that operators never need to run a compiler locally.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Rely on the `generate_all` macro. If you encounter missing package errors during CI, it indicates you are attempting to use non-deterministic Go logic that the Sovereign Mesh prohibits.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: TinyGo Compilation Pipeline Diagram ]</span>
            </div>
