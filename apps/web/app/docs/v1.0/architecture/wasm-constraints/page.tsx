...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Architecture: WASM Constraints`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Memory strictly bounded to 64 pages (4MB).</li>
                    <li>Payloads capped at 2MB maximum size.</li>
                    <li>System calls actively stubbed to `ErrNoSys` for determinism.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">To guarantee that untrusted code cannot abuse operator resources or break consensus via non-determinism, the WASM execution sandbox is severely constrained. By removing network access, file system access, clock functions, and arbitrary memory expansion, we force workloads into a pure functional state—enabling flawless Quorum matching.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">WASM Module instantiation → Wazero constraint validation → Pre-allocation of 64 pages → Execution → Immediate garbage collection of pages upon exit.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Explicit Constraint Definition
var WazeroConfig = wazero.NewModuleConfig().
  WithMemoryLimitPages(64). // 4MB Hard Limit
  WithSysWalltime().        // Stubbed to 0
  WithSysNanotime().        // Stubbed to 0
  WithSysNanosleep()        // Always returns ErrNoSys`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If a WASM module contains an instruction attempting to dynamically grow memory beyond the 64-page limit, the Wazero runtime will instantly emit an Out of Memory (OOM) trap, cancelling execution and failing the job deterministicly.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">WASM constraints are not mere recommendations; they are cryptographically enforced invariants. Modification of these constraints on a local operator node guarantees that the resulting execution hash will diverge from the compliant Quorum.</p>
  ...
  "event": "wasm_execution_metrics",
  "peak_memory_pages": 42,
  "wasi_traps": 0,
  "status": "clean_exit"
}`}</code></pre>
                    </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Interaction with Orchestrator over WSS multiplex channels. Execution RAM is strictly zeroed post-execution via `mem::forget` equivalents; telemetry is cached transiently and persisted selectively by Orchestrator.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Keep state logic minimal. If your computation requires multi-megabyte data sets, restructure the architecture to utilize MapReduce sharding patterns rather than attempting to bypass the 4MB memory boundary.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: WASM Resource Bounds Diagram ]</span>
            </div>
