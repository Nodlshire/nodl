...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Quorum & Slashing Mechanics`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Quorum Threshold: [TBD: Likely 2/3].</li><li>Slashing via Reputation reduction + cooldown.</li><li>Appeal windows for slashed operators.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Robust consensus is required to maintain the Deterministic Execution Model across a decentralized mesh of untrusted Tier 1 and Tier 3 operators. Slashing disincentivizes anomalous results.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Quorum Validation Logic
func ValidateQuorum(results []Result) error {
  if consensus < requiredThreshold { return ErrQuorumFailed }
  return slashAnomalous(results)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Failure to achieve 2/3 consensus stalls the job payload, triggering a retry across an isolated operator shard.</p>

            {/* 6. Invariants */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Invariants</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">WASM binary size = 2MB max. WASM memory = 64-pages (4MB). Cold start <10ms. Execution timeout = declared in spec.yaml.</p>
  ...
  "cooldown_ms": 86400000
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Orchestrator correlates Quorum mismatch metrics, applying immediate network isolation to flagged operators while emitting alerts.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Always handle non-deterministic external calls via the provided Oracles rather than embedding ErrNoSys-violating host extensions.</p>

            {/* 10. Appendix */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Appendix: Operator Scoring</h2>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-800">
                            ...
                            <th className="p-[12px] font-semibold text-[#f9fafb] text-[14px]">Impact</th>
                        </tr>
                    </thead>
                    <tbody className="text-[14px] text-slate-300 divide-y divide-slate-800">
                        <tr className="bg-[#0f1117] hover:bg-slate-900/50">
                            <td className="p-[12px]">Hash Mismatch</td>
                            <td className="p-[12px]">Slashing: [TBD: Reputation score reduction + cooldown periods]</td>
                        </tr>
                        <tr className="bg-slate-900/30 hover:bg-slate-900/50">
                            <td className="p-[12px]">Timeout Limit Exceeded</td>
                            <td className="p-[12px]">Reputation decay</td>
                        </tr>
                        <tr className="bg-[#0f1117] hover:bg-slate-900/50">
                            <td className="p-[12px]">WEX Interactions</td>
                            <td className="p-[12px]">[TBD: WEX staking, rewards, and full economics (Phase 3)]</td>
                        </tr>
                    </tbody>
                </table>
            </div>
