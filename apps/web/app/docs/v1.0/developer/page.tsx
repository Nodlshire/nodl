...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Developer: E2E Workflow`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Declarative generation via the `spec.yaml` interface.</li>
                    <li>Automated translation from high-level spec to deterministic WASM artifact.</li>
                    <li>Immutable artifact publishing to the global Integration Registry.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">To ensure security and compatibility across the Sovereign Mesh, developers cannot manually inject arbitrary WASM binaries. The ecosystem enforces a strict "Generation-First" development lifecycle. By starting with a strongly-typed `spec.yaml`, the system automatically generates the necessary Go stubs, JSON Schemas, and SDK wrappers, completely removing boilerplate and bridging the gap between intent and execution.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Define `spec.yaml` → Run `intgen` CLI → Implement Generated Go Stubs → Run `make build` (TinyGo Compilation) → Run `make test` (Wazero Sandbox Validation) → CI Cryptographically Signs Payload → Publish to Registry.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    <div className="px-4 py-2 border-b border-slate-800 bg-[#0f1117] flex justify-between items-center">
                        <span className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">yaml</span>
                    </div>
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`# Phase 1: The spec.yaml Foundation
...
inputs:
  - name: "collateral_ratio"
    type: "uint64"`}</code></pre>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If a developer attempts to bypass `intgen` by modifying the generated `.pb.go` or JSON schemas manually, the CI pipeline's checksum validation will aggressively fail the build. The generated code is strictly ephemeral and must only be manipulated via the `spec.yaml` definition.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The developer workflow relies on deterministic toolchains. Developers MUST use the exact toolchain versions specified in the repository's `go.mod` (e.g. TinyGo 0.30+) to ensure their locally generated WASM hashes match the CI-generated signatures exactly.</p>
  ...
  "event": "ci_pipeline_success",
  "integration_name": "liquidate_position",
  "wasi_compliance_check": "PASS",
  "registry_status": "published"
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Once successfully generated and signed by the CI Pipeline, the artifact is immutable. The Integration Registry indexes the schema, allowing the Orchestrator to securely ingest payloads formatted against it and seamlessly route them to the Mesh.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Commit your `spec.yaml` frequently and allow the `generate_all` makefile target to run automatically via pre-commit hooks. Never check in modified generated stubs.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: End-to-End Developer Workflow Diagram ]</span>
            </div>
