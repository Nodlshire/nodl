...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Developer: Testing & Debugging`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Utilize `TESTNET` flags to mock CI signatures locally.</li>
                    <li>Reproduce WASI traps locally using the `wnode-sim` CLI.</li>
                    <li>No remote SSH access to live edge operators; debugging is strictly local.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Because production execution happens remotely across thousands of anonymous operators, standard debugging workflows (like attaching Delve or executing SSH `fmt.Println` traps) are impossible. The testing framework requires developers to simulate the strict memory constraints and execution topology locally before pushing to the global pipeline.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Run Local Tests (`go test`) → Compile with TinyGo → Run Simulation via `wnode-sim` → Identify Memory/Timeout Traps → Optimize Code → Push to CI.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`# Simulating the remote Orchestrator environment locally
make build
wnode-sim execute --wasm build/module.wasm --mock-input mock.json --enforce-64-pages`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If you push a binary without local testing, and it contains an out-of-bounds array access, the live mesh will simply trap the error and report a generalized `422 Quorum Rejection` back to your client. You will not receive a stack trace from the edge operator. Local simulation is the ONLY way to trace panics.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The local testing sandbox (`wnode-sim`) implements the exact same Wazero constraints as the production daemon. A successful local execution guarantees a successful remote execution, assuming inputs are identical.</p>
  ...
  "status": "PASS"
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Local testing completely bypasses the Ingestion Gateway and Orchestrator. It acts as a direct, isolated sandbox mimicking the exact conditions of a single Tier-1 Earth Mesh operator.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Integrate `wnode-sim` directly into your GitHub Actions or local Git pre-push hooks to verify that modifications to your `spec.yaml` don't exceed the memory or binary boundaries.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Local Sandbox Testing Diagram ]</span>
            </div>
