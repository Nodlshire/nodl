...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Developer: Toolchain Requirements`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Strict requirement on TinyGo 0.30+ for WASI compatibility.</li>
                    <li>Go 1.21+ required for deterministic toolchain features.</li>
                    <li>Standard Go compilation (`GOOS=js`) is inherently rejected.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Consistency across the mesh requires a highly unified compiler pipeline. Standard Go's WASM implementation injects massive runtime overheads (garbage collectors, goroutine schedulers) pushing binary sizes near 15MB. The TinyGo toolchain acts as our mandated LLVM frontend, stripping dead code and ensuring payload compactness to respect the 2MB bounds.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Developer commits code → CI boots Go 1.21+ image → TinyGo 0.30+ executes `-target=wasi` → Strip debugging symbols → Emit 2MB conformant artifact.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`# Mandatory Compilation Target via Makefile
tinygo build -o build/main.wasm \
  -target=wasi \
  -no-debug \
  -panic=trap \
  ./cmd/integration`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If you manually attempt to compile the integration using standard `go build`, the artifact will instantly fail the `wnode-sim` testing phase due to size violations (e.g. producing an 11MB file). Furthermore, the Orchestrator will reject the payload signature since the CI pipeline enforces TinyGo exclusively.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The toolchain is absolutely inflexible. Do not submit pull requests suggesting the inclusion of Rust or AssemblyScript for integrations; the global CI orchestration and generated SDK stubs are exclusively bound to the TinyGo target model.</p>
  ...
  "event": "toolchain_validation",
  "compiler": "tinygo_0_30_0",
  "target": "wasi",
  "result": "verified"
}`}</code></pre>
                    </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The output produced by the TinyGo toolchain is heavily optimized to be instantly ingested by the Wazero runtime residing on the Operator daemon. The synergy between TinyGo's LLVM emissions and Wazero's AOT compiler guarantees the sub-10ms performance.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Use `goenv` or Docker to manage your local Go versions. Relying on your host system's default package manager often results in out-of-date TinyGo toolchains that fail the WASI compilation target required by the repository Makefile.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Toolchain Compilation Pipeline Diagram ]</span>
            </div>
