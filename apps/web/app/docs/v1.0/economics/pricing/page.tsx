...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Economics: Execution Pricing`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Prices derived from exact WASI instruction counts (Proof of Compute).</li>
                    <li>Tier multipliers applied based on requested latency (Tier-1 Earth Mesh costs more).</li>
                    <li>Predictable, deterministic cost per execution.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">In standard cloud environments, you rent time (e.g. EC2 hourly billing) or opaque invocation bounds (e.g. Lambda ms). The Sovereign Mesh bills based on mathematical determinism. Because the same WASM code will execute the exact same number of WASI instructions on every operator, the cost of execution is 100% predictable prior to deployment, eliminating bill shock.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Developer runs Local Sim → Outputs WASI Instruction Count → Orchestrator Multiplies by Tier Rate → Applies Bloat Multiplier → Deducts USD equivalent per job.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Deterministic Billing Calculation
func CalculatePrice(instructions uint64, tier Tier, bloatMult float64) uint64 {
  baseCost := instructions * CostPerInstruction
  tierCost := baseCost * GetTierMultiplier(tier)
  return uint64(float64(tierCost) * bloatMult)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If an integration contains an unexpected heavy iteration loop (e.g., parsing a massive array that scales linearly with user input), the WASI instruction count will spike, leading to higher billing per execution. Developers must bound their inputs strictly within the `spec.yaml` to prevent malicious clients from intentionally spiking execution costs.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The `CostPerInstruction` is a globally set parameter. It does not fluctuate based on network demand (no dynamic gas fees), ensuring enterprise clients can project infrastructure budgets quarterly.</p>
  ...
  "event": "execution_billed",
  "wasi_instructions": 45020,
  "tier_requested": "tier-1",
  "total_usd_cents": 0.005
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The operator daemon automatically tracks WASI instruction counts via the Wazero telemetry hook. It flushes this number back to the Orchestrator. Because determinism requires all nodes to report the exact same number of instructions, the Orchestrator uses the Quorum-agreed instruction count to calculate the bill.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Always optimize algorithms for Time Complexity (Big O). A quadratic `O(N^2)` algorithm will result in quadratically scaling instruction counts, drastically increasing your monthly execution costs for large input arrays.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Tiered Pricing Multiplier Table ]</span>
            </div>
