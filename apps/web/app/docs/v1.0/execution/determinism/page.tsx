        ...
        <div className="w-full pb-24">
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Identical inputs mathematically guarantee identical outputs.</li>
                    <li>Elimination of environment-specific drift.</li>
                    <li>Essential for achieving trustless Quorum consensus.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">In a decentralized Sovereign Mesh, computation is distributed across highly heterogeneous hardware running various operating systems. If a function's output varies based on CPU architecture, clock speed, or underlying OS libraries, Quorum consensus is broken. Determinism is the absolute foundation of cryptographic verification; without it, operator signatures hold no objective meaning.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Orchestrator Broadcasts Input State → Operators N independently execute WASM → Operators independently generate output hash → Orchestrator aggregates hashes → If Hash(N) == Hash(N+1), consensus is achieved.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Stubbing Non-Deterministic System Calls
func DeterministicWASI() wazero.ModuleConfig {
  return wazero.NewModuleConfig().
    ...
    WithSysNanotime(func() int64 { return 0 })
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If an operator manages to inject environment variables, localized file system state, or host time into the execution container, their resultant output hash will diverge from the Quorum. The Orchestrator treats this divergence as a Byzantine fault, triggering an automated slash against the diverging operator.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Functions must be strictly pure. All dependencies must be injected as parameters within the input spec. External network fetches and asynchronous disk reads are fundamentally restricted.</p>
  ...
  "determinism_confidence": 1.0
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The compiler pipeline ensures determinism syntactically via AST generation. The Wazero runtime enforces it physically via syscall stubs. The Orchestrator validates it cryptographically via Quorum hash matching.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Never use pseudo-random number generators seeded by time or memory addresses. If entropy is required, it must be explicitly provided in the `spec.yaml` as an execution parameter by the Orchestrator.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Deterministic Execution Path Diagram ]</span>
            </div>
