        ...
        <div className="w-full pb-24">
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Strict trapping of out-of-bounds memory accesses.</li>
                    <li>Context timeouts enforce hard SLA caps on runaway execution.</li>
                    <li>Quorum divergence results in immediate fault localization.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Executing untrusted WASM payloads at scale necessitates aggressive failure mode mapping. The Sovereign Mesh does not attempt to salvage faulty execution; instead, it enforces a fail-fast architecture. By immediately trapping memory violations, infinite loops, and stack overflows, we protect operator infrastructure and allow the Orchestrator to quickly re-route the failed job specification to a healthy Quorum.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Execution Fault Detected (e.g., Segfault) → Wazero Traps Execution → Operator emits explicit Trap Event over WSS → Orchestrator flags Node/Payload → Job Scheduled for Re-execution (if applicable).</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Capturing Execution Failures
if err := runWithTelemetry(mod); err != nil {
  if sysErr, ok := err.(*sys.ExitError); ok {
    return handleTrap(sysErr)
  }
  return err // Return raw timeout or context errors
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Execution failure modes are highly structured: Out Of Memory (OOM), Instruction Timeout, Unauthorized Syscall, and Stack Overflow. Each mode has a deterministic integer representation ensuring that when operators trap, they reach consensus on exactly *how* the payload failed, not just that it failed.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">A failure in one isolated WASM sandbox must NEVER propagate to the host operator OS. Cgroups v2 limits act as a secondary hardware-level invariant preventing host resource starvation in the event of an engine-level trap failure.</p>
  ...
  "node_id": "operator-v2-48xx"
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">When a local node traps, it zeroes its memory and flushes the error via the WSS channel to the Orchestrator. The Orchestrator aggregates these failure states; if the entire Quorum returns the identical `ERR_OOM` trap, the payload is correctly classified as malformed and the client is notified, avoiding operator slashes.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Ensure your CI integration tests thoroughly simulate out-of-bound edge cases to prevent deterministic traps occurring on the live production Mesh, as frequent payload trapping negatively affects routing reputation.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Execution Fault Matrix Diagram ]</span>
            </div>
