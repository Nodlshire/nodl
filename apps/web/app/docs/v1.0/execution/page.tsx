...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Execution: Pipeline Architecture`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Abstract Syntax Tree (AST) compilation directly to deterministic WASM.</li>
                    <li>Zero-trust Wazero sandbox restricts memory and syscall access.</li>
                    <li>Synchronous telemetry emission embedded in execution lifecycle.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">To guarantee deterministic execution across a decentralized topology, human error must be eliminated from the build process. The Sovereign Mesh utilizes an automated execution pipeline that translates declarative AST schemas into hardened WASM binaries. By enforcing execution exclusively within a locked-down Wazero sandbox, the system isolates operator host hardware from malicious or runaway payloads.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">YAML Spec → AST Parsing → TinyGo Compilation → Code Signing → Operator Ingress → Sandbox Pre-allocation → Execution & Context Isolation → Zero-Overhead Telemetry Emission → Memory Teardown.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Immutable Execution Wrapper
func ExecutePipeline(wasmBytes []byte) (Result, error) {
  // Enforce zero-trust sandbox boundaries
  mod, err := wazero.Instantiate(ctx, wasmBytes, WazeroConfig)
  if err != nil { return nil, err }
  defer mod.Close(ctx) // Deterministic memory zeroing
  return runWithTelemetry(mod)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Execution terminates abruptly if the payload triggers a Wazero segmentation fault, attempts forbidden syscalls (e.g. `os.Exit`), or exceeds the strictly defined timeout boundary. Any abnormal termination emits a deterministically structured trap error, ensuring quorum can still be reached on the failure state.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Execution must be functionally pure. Payloads cannot import non-deterministic packages. Time, RNG, and network I/O are explicitly blocked at the compiler layer and further stubbed at the runtime layer.</p>
  ...
  "result_hash": "e3b0c...",
  "wasi_instruction_count": 48201,
  "telemetry_flush_ms": 0.5
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The execution pipeline natively wraps all WASM operations with lightweight telemetry hooks. These hooks extract metrics (instruction counts, memory peaks) synchronously before immediately returning control to the node for asynchronous Orchestrator flushing.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Developers should never rely on shared state or persistent execution variables between function calls. Every execution is a cold start instantiated into a pristine memory space.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Execution Pipeline AST/WASM Diagram ]</span>
            </div>
