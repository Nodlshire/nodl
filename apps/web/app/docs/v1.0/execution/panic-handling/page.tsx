...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Execution: Panic Handling`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Strict host-level `recover()` implementation around WASM entrypoints.</li>
                    <li>Panics within WASM inherently yield deterministic execution hashes.</li>
                    <li>Host process termination is strictly prevented via panic isolation.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">In standard Go applications, an unhandled panic crashes the entire process. On a decentralized mesh, an operator crashing due to a malformed client payload is an unacceptable security vulnerability, creating an easy DDoS vector. The execution pipeline strictly sandboxes panics both within the WASM payload via compiler traps and at the host level via rigorous `recover()` boundaries, ensuring node uptime.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">WASM Payload Triggers Panic → TinyGo Runtime converts Panic to Trap → Wazero Catches Trap → Host `recover()` barrier logs error → Orchestrator informed of 422 Quorum Rejection.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Host-Level Panic Isolation
defer func() {
  if r := recover(); r != nil {
    log.Errorf("Recovered from panic during WASM execution: %v", r)
    emitTrapTelemetry(r)
  }
}()`}</code></pre>
                    </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If an operator alters the host binary and removes the `recover()` boundary, a malformed payload will crash the process. The operator will drop offline, failing to submit their hash to the Orchestrator. This missed deadline results in an immediate reduction in their routing reliability score.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Host processes must remain online. WASM payloads compiled via `generate_all` are hardcoded to use `-panic=trap`, ensuring that internal payload panics never attempt complex stack unwinding that could induce memory leaks in the Wazero environment.</p>
  ...
  "event": "operator_panic_recovered",
  "reason": "index_out_of_bounds",
  "wasi_trap_triggered": true
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">When a panic is trapped, the operator node halts execution, zeroes the local memory buffer, and securely flushes the telemetry via WSS to the Orchestrator, marking the execution as deterministically failed.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Developers should explicitly handle bounds checking in their declarative logic. Relying on panics as a standard error return mechanism is a severe anti-pattern that bloats execution time and telemetry pipelines.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Panic Isolation Diagram ]</span>
            </div>
