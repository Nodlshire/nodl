        ...
        <div className="w-full pb-24">
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Strict 64-page (4MB) max memory allocation per WASM execution.</li>
                    <li>Artifact binary sizes capped firmly at 2MB.</li>
                    <li>Dual-layered bounding via Wazero config and Host OS cgroups v2.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">In an open compute market, resource exhaustion attacks (e.g. memory inflation, fork bombs) are trivial attack vectors. By capping binary sizes at 2MB, we guarantee fast network propagation across the Sovereign Mesh. By clamping runtime memory to 64 pages (4MB), we ensure that a single malicious or inefficient payload cannot starve the bare-metal operator's host OS of resources.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Payload Ingress → Orchestrator Verifies Size {"<"}2MB → Operator Allocates 64 Pages → Execution Exceeds Allocation → OOM Trap Triggered → Execution Halts Safely.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Enforcing Hard Memory Boundaries
func CreateRuntimeConfig() wazero.ModuleConfig {
  // Exactly 64 pages of WebAssembly memory (1 page = 64KB)
  // Total allocation: 4MB Max
  return wazero.NewModuleConfig().WithMemoryLimitPages(64)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Attempting to compile a complex Go standard library package will often exceed the 2MB artifact limit, resulting in CI pipeline rejection. At runtime, attempting to dynamically allocate a large array that breaches the 4MB limit results in an immediate, safe OOM trap, preventing OS-level paging delays.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Memory allocations are fixed and immutable. Dynamic scaling or auto-expansion of memory pages based on compute demand is strictly prohibited, as it introduces non-determinism in execution time and hardware capability.</p>
  ...
  "event": "resource_monitor_update",
  "artifact_bytes": 1450000,
  "peak_memory_pages_used": 32,
  "memory_headroom_pages": 32
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Gateway layer enforces the 2MB artifact cap before Orchestrator ingestion. The Wazero Sandbox enforces the 64-page limit at runtime. Host OS cgroups v2 enforce CPU and systemic RAM constraints to ensure the operator node's underlying daemon remains stable.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Employ MapReduce architectures for large datasets. If a processing job requires analyzing 50MB of data, it must be sharded across multiple payloads by the Orchestrator to respect the 4MB memory boundary.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Resource Bounding Architecture Diagram ]</span>
            </div>
