...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Execution: Timeouts & Concurrency`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Dynamic context deadlines enforced via Go context cancellation.</li>
                    <li>Sub-10ms strict cold-start initialization boundary.</li>
                    <li>WASM payloads are single-threaded; concurrency is managed at Orchestrator level.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Computation that never finishes is indistinguishable from computation that is broken. Timeouts establish hard boundaries on execution cycles to prevent intentional infinite loop attacks (halting problem). Concurrency is strictly prohibited within the WASM execution itself to guarantee absolute determinism, moving all parallel orchestration logic out to the global state layer.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Payload Ingress → Context created with SLA Timeout → WASM Execution Begins → Timeout Reached → Wazero Execution Halted → Error Transmitted.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Context-Aware Execution Timeout
func ExecuteWithTimeout(ctx context.Context, job Job) error {
  // Extract SLA declared in spec.yaml
  execCtx, cancel := context.WithTimeout(ctx, job.TimeoutDuration)
  defer cancel()
  
  return wazero.InstantiateAndRun(execCtx, job.Payload)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If execution exceeds the defined timeout, the context is cancelled, which immediately interrupts the Wazero instruction pointer. A timeout is logged as a specific trap, enabling the Quorum to safely agree that the execution exceeded SLA bounds without crashing the operators.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">WASM binaries must execute synchronously and single-threadedly. Initiating internal goroutines within the TinyGo payload is blocked, ensuring CPU scheduling determinism.</p>
  ...
  "event": "execution_timeout_triggered",
  "sla_budget_ms": 100,
  "actual_duration_ms": 100.01,
  "status": "terminated"
}`}</code></pre>
                    </div>
                </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Timeouts are established globally by the Orchestrator via the `spec.yaml` definition and enforced locally by the Operator's host binary. Concurrency is orchestrated by the global routing tier mapping shards to different operators simultaneously.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Set aggressive SLA timeouts in your spec definitions. An execution that regularly takes 95ms on a 100ms budget is at high risk of trapping across the quorum due to minor environmental jitter. Always provide at least a 30% execution buffer.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Timeouts and Context Cancellation Diagram ]</span>
            </div>
