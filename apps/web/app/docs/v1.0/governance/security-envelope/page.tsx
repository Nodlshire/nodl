...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Governance: Security Envelope`}</h1>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Security assumes malicious operators. The Security Envelope uses Ingress HMAC and Quorum validation.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Request -&gt; Ingress HMAC -&gt; Nonce check -&gt; Distribute to Quorum -&gt; Compare Hashes -&gt; Settle.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`if !hmac.Equal(sig, expected) { return ErrUnauthorized }`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Security Envelope enforced: Ingress HMAC, Nonce + replay protection, Ed25519 signatures, Quorum validation, Operator slashing [TBD: Reputation score reduction + cooldown periods], Telemetry authenticity.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">All artifacts MUST bear valid Ed25519 signature from DAO deployer.</p>
