...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Operator: Tier Archetypes`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Tier-1: High-frequency bare-metal operators (Earth Mesh).</li>
                    <li>Tier-2: General-purpose VPS infrastructure (Earth Mesh).</li>
                    <li>Tier-3: Asynchronous, highly resilient LEO nodes (Space Mesh).</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Mesh is heterogenous by design. A satellite operator cannot process a 50ms algorithmic trade, and a localized fiber-connected server cannot provide global censorship resistance during terrestrial network partitions. By categorizing operators into strict archetypes, the Orchestrator can intelligently map workloads to the exact hardware profile required by the execution spec.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Operator Boots → Benchmarks Hardware Capabilities → Requests Tier Classification → Orchestrator Validates Latency Profile → Operator joins specific Shard Routing Table.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Tier Classification Logic
func ClassifyOperator(ping time.Duration, isSatellite bool) Tier {
  if isSatellite { return Tier3 }
  if ping < 15*time.Millisecond { return Tier1 }
  return Tier2
}`}</code></pre>
                    </div>
                </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Operators constantly experience network jitter. If a Tier-1 operator's average ping drops below the required SLA threshold, the Orchestrator automatically demotes them to Tier-2, assigning them lower-value, less time-critical tasks until their network stabilizes.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Operators cannot self-report their tier. The Orchestrator derives classification exclusively through active cryptographic ping measurements and historical payload completion data.</p>
  ...
  "avg_latency_ms": 12.4
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Orchestrator's Routing Engine strictly filters its queries by the requested execution tier. Client jobs utilizing `spec.yaml` can explicitly demand Tier-1 routing for financial workloads, or accept Tier-2 for standard data processing to save on gas.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Operators aiming for Tier-1 status should deploy on bare-metal infrastructure physically proximate to major IXPs (Internet Exchange Points). VPS providers often introduce virtualization jitter that makes sustained Tier-1 classification impossible.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Operator Archetype Distribution Diagram ]</span>
            </div>
