...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Operator: Earth Mesh Guidelines`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Requires 99.99% uptime and persistent WebSocket connections.</li>
                    <li>Hardware must support strict cgroups v2 resource dedication.</li>
                    <li>Designed for high-frequency, low-latency task execution.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Earth Mesh operators bear the brunt of the network's synchronous workloads. Because these tasks (like decentralized exchange liquidations) are time-sensitive, the operator infrastructure must be rigorously maintained. Operating a Tier-1 Earth Mesh node is a professional commitment to bare-metal performance, bypassing virtualization overheads.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Bare-Metal OS Init → Systemd configures Cgroups v2 → `nodld` starts → Establishes WSS → Maintains {"<"}15ms Ping → Rapid Execution → Instant Hash Flush.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`# Example systemd unit snippet enforcing cgroups
[Service]
ExecStart=/usr/local/bin/nodld
MemoryMax=8G
CPUQuota=400%
Slice=machine.slice`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If an Earth Mesh operator fails to utilize cgroups to restrict the daemon, a single maliciously constructed (though syntactically valid) memory-heavy payload across many parallel invocations could trigger host OS OOM killers. The node will crash, losing all WSS state and suffering heavy economic slashing.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Earth Mesh routing relies strictly on persistent WSS. Polling is blocked. Any node that attempts to revert to HTTP polling will be denied payloads.</p>
  ...
  "event": "operator_health_ping",
  "tier": 1,
  "cgroup_memory_usage_pct": 42.1,
  "wss_latency_ms": 11.2
}`}</code></pre>
                    </div>
                </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The operator relies on the local OS kernel to enforce resource limits (cgroups), ensuring that the `nodld` process, which instantiates Wazero memory pools, can never exceed the boundaries agreed upon during operator registration.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Operators should deploy on Linux distributions utilizing Kernel 5.15+ to ensure robust cgroups v2 support. Avoid running unrelated heavy workloads (e.g., database instances) on the same host machine to prevent OS scheduler jitter.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Earth Mesh Node Topology Diagram ]</span>
            </div>
