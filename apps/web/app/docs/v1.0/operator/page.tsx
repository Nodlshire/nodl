...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Operator: Node Lifecycle`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Single static daemon (`nodld`) manages all execution, networking, and telemetry.</li>
                    <li>Automated registration and cryptographically verifiable identities.</li>
                    <li>Continuous background heartbeat monitoring via WSS.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Node Operators are the physical bedrock of the Sovereign Mesh. To ensure maximum decentralization, the barrier to entry must be minimal. A single, self-contained binary encapsulates the entire security envelope, WASM runtime, and routing logic, ensuring that operators focus purely on hardware uptime rather than complex software orchestration.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Key Generation → Orchestrator Registration Handshake → WSS Connection Established → Idle Polling → Payload Received → Local Signature Verification → WASM Execution → Telemetry/Hash Flush.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Core Operator Daemon Loop
func (o *Operator) Start(ctx context.Context) error {
  if err := o.RegisterIdentity(); err != nil { return err }
  
  conn := o.establishWSS()
  go o.maintainHeartbeat(ctx, conn)
  return o.listenForJobs(ctx, conn)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If an operator fails to maintain its WSS heartbeat, or if its hardware clock drifts significantly, the Orchestrator will aggressively sever the connection. The node is marked offline and its reputation score drops. Persistent failures will result in the node being culled from the routing tables entirely, requiring manual re-registration.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Operators must never modify the `nodld` binary. Any attempt to alter the daemon's internal execution pipeline or falsify telemetry will result in mismatched payload hashes and immediate economic slashing.</p>
  ...
  "jobs_processed": 5420
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Operator interacts solely with the Global Orchestrator via highly structured JSON schemas over WebSockets. P2P communication between operators is explicitly banned to eliminate Sybil attack vectors and collusion during Quorum generation.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Always utilize systemd or a similar init system to manage the daemon. Relying on detached shell sessions ensures the node will fail to recover cleanly upon a host OS reboot.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Operator Node Architecture Diagram ]</span>
            </div>
