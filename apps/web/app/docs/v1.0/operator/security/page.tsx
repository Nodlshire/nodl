...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Operator: Security Boundaries`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Operators actively reject unverified Ed25519 payloads.</li>
                    <li>WASM runtime is entirely isolated from the host OS context.</li>
                    <li>Zero-knowledge design: payloads cannot access local file descriptors.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The operator must operate in a zero-trust environment. Operators do not trust the Orchestrator, the network, or the execution payloads. They rely exclusively on cryptographic math. If the network is compromised and a malicious payload is delivered, the operator's host binary must recognize the bad signature and sever the connection before any memory is allocated.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Payload Ingress → Local Ed25519 Validation → If valid: Allocate Wazero → If invalid: Drop & Alert → Execute without OS Syscalls.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Operator Side Cryptographic Gate
func (o *Operator) AcceptJob(job Job) error {
  if !VerifyEnvelope(job.Payload, globalCIKey, job.Signature) {
    return errors.New("security envelope violation: signature invalid")
  }
  return o.execute(job)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">If an operator's local public key registry becomes desynchronized, they will begin rejecting valid payloads. This results in the Orchestrator perceiving the node as faulty, leading to temporary routing table suspension until the operator resyncs their cryptographic state.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The `generate_all` pipeline is the sole authority for creating payload signatures. Edge operators are forbidden from compiling or signing code locally. They are strictly execution engines.</p>
  ...
  "event": "operator_local_validation",
  "result": "rejected",
  "reason": "ed25519_mismatch",
  "payload_hash": "ff22a1..."
}`}</code></pre>
                    </div>
                </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The Operator validates the CI-generated signature that was relayed by the Orchestrator. This ensures end-to-end immutability from the developer's AST schema all the way to the edge hardware's memory buffer.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Operators should ensure their system clocks are properly synchronized via NTP. Severe clock drift can sometimes interfere with token-based WSS authentication handshakes at the Orchestrator layer, even if payload signatures are static.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Operator Sandbox Security Diagram ]</span>
            </div>
