...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Overview: Core Code`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Single, strongly-typed execution entrypoint.</li>
                    <li>Cryptographic validation via Ed25519 signature checks.</li>
                    <li>Wazero integration with strictly constrained WASI definitions.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">By routing all disparate operational tasks through a single, standardized core code pipeline, we eliminate edge cases and configuration drift. This rigid structure guarantees that any WASM binary executing on the mesh adheres to deterministic resource and time constraints, preventing runaway workloads and protecting operator hardware.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Payload Ingress → Ed25519 Auth Verification → Orchestrator Routing Table Lookup → WASM Payload Instantiation → Execute via Wazero → State Verification → Quorum Settlement.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Immutable Core Handler Execution
func Handle(payload []byte, sig []byte) (Result, error) {
  if err := VerifyEd25519(payload, sig); err != nil {
    return nil, ErrAuthFailed
  }
  return wazero.InstantiateAndRun(payload)
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">A primary failure mode occurs if execution exceeds the hardcoded timeout (10ms initialization, plus the job-specific execution limit declared in spec.yaml). In this scenario, the context is cancelled, and a timeout metric is flushed to the Orchestrator.</p>
            ...
            <div className="my-[24px] p-[12px] bg-amber-500/10 border-l-4 border-amber-500 rounded-r-lg">
                ...
                <p className="text-[14px] text-slate-300 leading-[1.6] m-0">Uncaught panics within the core loop yield an immediate 422 Quorum Rejection and trigger Operator slashing [TBD: Reputation score reduction + cooldown periods].</p>
            </div>

            {/* 6. Invariants */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Invariants</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Core pipeline operations cannot allocate memory dynamically outside of the pre-defined 64-page boundary. System calls are explicitly mapped to ErrNoSys.</p>
  ...
  "metric": "core_execution_time_ms",
  "value": 4.12,
  "status": "success",
  "wasi_calls_blocked": 0
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The core execution engine interfaces directly with the Orchestrator via persistent WSS channels. Once a job is completed, execution RAM is synchronously wiped to prevent state leakage. Direct P2P communication between operators remains structurally impossible.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Developers should never attempt to bypass the core handler by injecting custom execution loops. Always rely on the CI pipeline's `generate_all` to wrap your business logic correctly.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Core Execution Pipeline Diagram ]</span>
            </div>
