...
export default function Page() {
    return (
        <div className="w-full pb-24">
            <h1 className="text-[28px] md:text-[32px] font-bold text-[#f9fafb] mb-[12px] leading-tight tracking-tight">{`Overview: Core Invariants`}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Mathematical execution determinism across heterogeneous hardware.</li>
                    <li>Zero-trust execution environment via strict WASI constraint mapping.</li>
                    <li>Sub-10ms initialization maximum strictly enforced at runtime.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Invariants are non-negotiable protocol constraints that guarantee systemic stability. Without strict invariants, a decentralized mesh devolves into non-deterministic state drift, rendering Quorum consensus mathematically impossible. These rules enforce uniformity regardless of the underlying OS or hardware architecture of the individual node operators.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Constraint Definition (spec.yaml) → Build-time Static Verification → Cryptographic Signing → Runtime Wazero Instantiation Check → Quorum Hashes.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Invariant Enforcement
func ConfigureWASI() wazero.ModuleConfig {
  // Strip all non-deterministic APIs
  return wazero.NewModuleConfig().
    WithSysNanosleep().
    WithSysWalltime().
    WithFS(nil) // Block all disk I/O
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Violating an invariant (e.g., attempting a blocked syscall or allocating memory beyond the allowed pages) instantly traps the WASM execution, emitting a deterministic failure hash to the Quorum.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The primary invariants are resource bounds: 64 pages (4MB) max memory, 2MB max artifact size, and strict zero-network access during execution. No host-side RNG or time data can be ingested.</p>
  ...
  "memory_limit_pages": 64
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Invariants are validated independently by both the Orchestrator (during artifact ingestion) and by individual operators (prior to Wazero instantiation). If an operator receives a payload that violates invariants, it must reject it locally without execution.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Always utilize the CI pipeline's `generate_all` rather than manually constructing payloads, as the CLI toolchain inherently enforces these invariants at compile time.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: System Invariants Diagram ]</span>
            </div>
