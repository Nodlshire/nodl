        ...
        <div className="w-full pb-24">
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-[12px] md:p-[16px] rounded-lg mb-[32px]">
                ...
                <ul className="text-[16px] leading-[1.6] text-slate-300 space-y-[8px] list-disc list-inside m-0">
                    <li>Strict deterministic isolation via TinyGo and WASM.</li>
                    <li>Declarative generation over imperative programming.</li>
                    <li>Sub-10ms initialization maximum to support high-frequency meshes.</li>
                </ul>
            </div>

            {/* 2. Rationale */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Rationale</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Nodes must never run arbitrary or mutable imperative code. The Sovereign Compute Mesh demands absolute determinism, which is achieved by compiling strictly defined declarative YAML schemas into WASM using TinyGo. This generative approach entirely removes developer error from the runtime layer, moving complexity left into the CI compilation stage.</p>

            {/* 3. Flow */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Flow</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">spec.yaml is committed → CI statically analyzes limits → `generate_all` synthesizes Go stubs → Compiled to TinyGo WASM → Deployed immutably to Artifact Registry.</p>
            ...
            <div className="mt-[20px] mb-[20px]">
                <div className="bg-[#0d1117] border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
                    ...
                    <div className="p-4 overflow-x-auto">
                        <pre className="text-[14px] font-mono text-[#e5e7eb] leading-[1.5]"><code>{`// Declarative Schema Representation
type Spec struct {
  Name   string         \`yaml:"name"\`
  Limits ResourceLimits \`yaml:"limits"\` // Enforced via code generation
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Attempting to introduce non-deterministic logic (e.g. relying on host time, threading, or hardware RNG) during development will fail at the compilation layer. If manually forced into a binary, it will cause Quorum rejection at runtime due to state divergence across edge operators.</p>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The `spec.yaml` is the ultimate source of truth. Manual modifications to the generated WASM are mathematically impossible to forge into the network due to required Ed25519 signatures from the CI authority.</p>
  ...
  "event": "spec_compiled_and_signed",
  "status": "success",
  "size_bytes": 460800,
  "signature": "ed25519:8b3a..."
}`}</code></pre>
                    </div>
                </div>
            </div>
            ...
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">The generated WASM artifacts represent the only payload standard accepted by the Orchestrator. The Orchestrator distributes these immutable payloads to operators over multiplex channels, guaranteeing identical byte execution globally.</p>

            {/* 9. Best Practices */}
            <h2 className="text-[22px] md:text-[24px] font-semibold text-[#f9fafb] mt-[32px] mb-[12px]">Best Practices</h2>
            <p className="text-[16px] leading-[1.7] text-[#e5e7eb] mb-[16px]">Always utilize the CI pipeline's `generate_all` rather than manually constructing payloads. Never attempt to reverse-engineer or manually compile standard Go binaries, as they will vastly exceed the 2MB limit and pull in non-deterministic runtime dependencies.</p>
            ...
            <div className="mt-[32px] mb-[16px] p-[24px] bg-slate-900 border border-slate-700 border-dashed rounded-lg flex items-center justify-center">
                <span className="text-slate-500 font-mono text-[14px]">[ Placeholder: Generative Substrate Diagram ]</span>
            </div>
