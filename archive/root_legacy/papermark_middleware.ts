  ...
  return (
    ...
    (process.env.NODE_ENV !== "development" &&
      !(
        ...
        host?.includes("dr.wnode.one") ||
        host?.includes(process.env.NEXT_PUBLIC_APP_BASE_HOST || "never")
      ))
  );
}
...
export const config = {
  matcher: [
     ...
     */
    "/((?!api/|_next/|_static|vendor|_icons|_vercel|favicon.ico|favicon-32x32.png|favicon-16x16.png|sitemap.xml).*)",
  ],
};
...
export default async function middleware(req: NextRequest, ev: NextFetchEvent) {
  ...
  console.log("MIDDLEWARE RUNNING. HOST IS:", host, "isCustomDomain:", isCustomDomain(host || ""));
