...
## 1. Executive Summary
The `chainlink` integration connects the Sovereign Mesh deterministically to the Chainlink Price Feeds. It acts as a mesh-safe gateway ensuring that all interactions with chainlink—such as fetching state, executing logic, or verifying proofs—are mathematically reproducible across nodes. By wrapping chainlink\'s RPC endpoints in a strict, pure-function adapter, the Sovereign Mesh guarantees that no non-deterministic execution paths can breach the consensus layer or corrupt the proof of compute.
...
- **Supported Networks**: [Multiple EVM Networks](/docs/integrations/multiple-evm networks)
- **Adapter Hash**: `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`
- **Last Updated**: 2026-06-28

## 3. Protocol Overview
- **Architecture**: The chainlink integration acts as the bridge for Oracle operations.
- **RPC Surfaces**: Uses EVM JSON-RPC to Chainlink AggregatorV3 interfaces. Strictly requires deterministic block/height bounds.
- **Data Models**: `RoundData` (roundId, answer, startedAt, updatedAt, answeredInRound).
- **Proof Models**: Ethereum state roots verifying Decentralized Oracle Network aggregation. The Sovereign Mesh independently verifies these roots via quorums.
...
- **Capability Map**: Returns `{ canFetch: true, canSubmit: false, canValidate: true }`
...
| `STALE_ORACLE` | Protocol-specific boundary violated for Chainlink. |
| `NEGATIVE_PRICE_DETECTED` | Protocol-specific boundary violated for Chainlink. |
| `AGGREGATOR_SUSPENDED` | Protocol-specific boundary violated for Chainlink. |
...
```json
{
  "integrationName": "chainlink",
  "integrationOperation": "fetch",
  "params": {
    "action": "latestRoundData", "aggregator": "0x...", "blockTag": "0x123"
  }
}
```
...
- **Enabling/Disabling**: Operators can toggle the integration via `nodl mesh integration disable chainlink`.
- **Configuring RPC Endpoints**: Configurable in `/etc/nodl/integrations/chainlink.yaml`.
...
`Workflow Assignment -> MeshWorker Sandbox -> IntegrationRegistry -> ChainlinkAdapter -> RPC (Block N) -> Decode -> Canonical JSON -> SHA-256 -> StepResult`

## 14. Testing & Validation
- **Test Suite**: Fully covered in `test/integrations/chainlink.test.ts` within the TS SDK.
- **Validation**: Mocks simulate healthy execution and strict error bounds.

## 15. Example Scenarios
- **Automated Workflow**: A sovereign workflow continuously fetches state. If thresholds are met, it transitions to a `submit` step via the Chainlink adapter.
- **Cross-Chain Aggregation**: A workflow reads from Chainlink and compares it with other chains to execute deterministic arbitrage or bridging.

## 16. References & Sources
- [Chainlink Official Documentation](https://docs.chainlink.io)
