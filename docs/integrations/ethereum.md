...
## 1. Executive Summary
The `ethereum` integration connects the Sovereign Mesh deterministically to the Ethereum Mainnet. It acts as a mesh-safe gateway ensuring that all interactions with ethereum—such as fetching state, executing logic, or verifying proofs—are mathematically reproducible across nodes. By wrapping ethereum\'s RPC endpoints in a strict, pure-function adapter, the Sovereign Mesh guarantees that no non-deterministic execution paths can breach the consensus layer or corrupt the proof of compute.

## 2. Verified Metadata Block
- **Integration Name**: Ethereum
- **Version**: 1.1.0
- **Determinism Profile**: Purely Deterministic (Block-Bound)
- **Capability Set**: Fetch, Submit, Validate
- **Supported Networks**: [Ethereum](/docs/integrations/ethereum) [Sepolia](/docs/integrations/sepolia)
- **Adapter Hash**: `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`
- **Last Updated**: 2026-06-28

## 3. Protocol Overview
- **Architecture**: The ethereum integration acts as the bridge for Blockchain operations.
- **RPC Surfaces**: Uses EVM JSON-RPC (`eth_call`, `eth_getLogs`). Strictly requires deterministic block/height bounds.
- **Data Models**: EVM `Transaction`, `Log`, `Block`.
- **Proof Models**: Ethereum Merkle Patricia Trie state roots. The Sovereign Mesh independently verifies these roots via quorums.
...
| `GAS_SPIKE_TIMEOUT` | Protocol-specific boundary violated for Ethereum. |
| `UNCLE_BLOCK_DETECTED` | Protocol-specific boundary violated for Ethereum. |
| `STATE_PRUNED` | Protocol-specific boundary violated for Ethereum. |
...
```json
{
  ...
  "params": {
    "action": "eth_call", "to": "0x...", "data": "0x...", "blockTag": "0x123"
  }
}
```
...
- **Enabling/Disabling**: Operators can toggle the integration via `nodl mesh integration disable ethereum`.
- **Configuring RPC Endpoints**: Configurable in `/etc/nodl/integrations/ethereum.yaml`.
...
`Workflow Assignment -> MeshWorker Sandbox -> IntegrationRegistry -> EthereumAdapter -> RPC (Block N) -> Decode -> Canonical JSON -> SHA-256 -> StepResult`

## 14. Testing & Validation
- **Test Suite**: Fully covered in `test/integrations/ethereum.test.ts` within the TS SDK.
- **Validation**: Mocks simulate healthy execution and strict error bounds.

## 15. Example Scenarios
- **Automated Workflow**: A sovereign workflow continuously fetches state. If thresholds are met, it transitions to a `submit` step via the Ethereum adapter.
- **Cross-Chain Aggregation**: A workflow reads from Ethereum and compares it with other chains to execute deterministic arbitrage or bridging.

## 16. References & Sources
- [Ethereum Official Documentation](https://docs.ethereum.io)
