...
## 1. Executive Summary
The `filecoin` integration connects the Sovereign Mesh deterministically to the Filecoin Storage Network. It acts as a mesh-safe gateway ensuring that all interactions with filecoin—such as fetching state, executing logic, or verifying proofs—are mathematically reproducible across nodes. By wrapping filecoin\'s RPC endpoints in a strict, pure-function adapter, the Sovereign Mesh guarantees that no non-deterministic execution paths can breach the consensus layer or corrupt the proof of compute.

## 2. Verified Metadata Block
- **Integration Name**: Filecoin
- **Version**: 1.1.0
- **Determinism Profile**: Purely Deterministic (Block-Bound)
- **Capability Set**: Fetch, Submit, Validate
- **Supported Networks**: [Filecoin](/docs/integrations/filecoin) [Calibration](/docs/integrations/calibration)
- **Adapter Hash**: `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`
- **Last Updated**: 2026-06-28

## 3. Protocol Overview
- **Architecture**: The filecoin integration acts as the bridge for Storage operations.
- **RPC Surfaces**: Uses JSON-RPC over HTTP to Lotus or GLIF nodes. Strictly requires deterministic block/height bounds.
- **Data Models**: `DealProposal`, `DealState`, `MinerInfo`.
- **Proof Models**: Filecoin Proof-of-Spacetime (PoSt) verifiable state roots. The Sovereign Mesh independently verifies these roots via quorums.
...
### Core Interfaces
- **Function Selectors**: 
`Filecoin.ChainGetTipSetByHeight(epoch)`
...
| `STALE_DEAL_STATE` | Protocol-specific boundary violated for Filecoin. |
| `SECTOR_FAULT_DETECTED` | Protocol-specific boundary violated for Filecoin. |
| `MINER_SLA_BREACH` | Protocol-specific boundary violated for Filecoin. |
...
```json
{
  "integrationName": "filecoin",
  "integrationOperation": "fetch",
  "params": {
    "action": "StateMarketStorageDeal", "dealId": 12345
  }
}
```
...
- **Enabling/Disabling**: Operators can toggle the integration via `nodl mesh integration disable filecoin`.
- **Configuring RPC Endpoints**: Configurable in `/etc/nodl/integrations/filecoin.yaml`.
...
`Workflow Assignment -> MeshWorker Sandbox -> IntegrationRegistry -> FilecoinAdapter -> RPC (Block N) -> Decode -> Canonical JSON -> SHA-256 -> StepResult`

## 14. Testing & Validation
- **Test Suite**: Fully covered in `test/integrations/filecoin.test.ts` within the TS SDK.
- **Validation**: Mocks simulate healthy execution and strict error bounds.

## 15. Example Scenarios
- **Automated Workflow**: A sovereign workflow continuously fetches state. If thresholds are met, it transitions to a `submit` step via the Filecoin adapter.
- **Cross-Chain Aggregation**: A workflow reads from Filecoin and compares it with other chains to execute deterministic arbitrage or bridging.

## 16. References & Sources
- [Filecoin Official Documentation](https://docs.filecoin.io)
