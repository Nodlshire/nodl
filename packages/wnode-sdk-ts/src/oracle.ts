...
import { WnodeOracleError } from './errors';
import { BlockTag, VerifiedPrice } from './types';
...
export interface GetVerifiedPriceOptions {
  ...
  deviationThreshold?: number; // e.g., 0.01 for 1%
}
...
export class OracleClient {
   ...
   */
  public async getVerifiedPrice(feedAddress: string, options?: GetVerifiedPriceOptions): Promise<VerifiedPrice> {
    const blockTag: BlockTag = options?.blockHash ? { blockHash: options.blockHash } : 'finalized';
    ...
    if (options?.secondaryFeedAddress) {
      ...
      if (deviation > deviationThreshold) {
        throw new WnodeOracleError('PRICE_MISMATCH', {
    ...
    if (options?.maxStaleness && staleness > options.maxStaleness) {
      throw new WnodeOracleError('STALE_ORACLE', {
        feed: feedAddress,
        staleness,
  ...
  private async _readFeed(feedAddress: string, blockTag: BlockTag): Promise<{ price: number; updatedAt: number; roundId: string }> {
    let result: any;
    try {
      ...
      throw new WnodeOracleError('READ_FAILED', {
        feed: feedAddress,
        error: err.message,
        chainId: this.client.chainId,
        timestamp: Math.floor(Date.now() / 1000),
    ...
    if (answer <= 0n) {
      throw new WnodeOracleError('INVALID_PRICE', {
        feed: feedAddress,
