...
func TestSandbox_DataCorrectnessEndToEnd(t *testing.T) {
	...
	// === VERIFY: Zero fake/synthetic data ===
	// Latitude/Longitude should be 0 since we didn't provide an IP
	if node.Latitude != 0 || node.Longitude != 0 {
		t.Errorf("Expected zero coords (no IP provided), got lat=%f lon=%f", node.Latitude, node.Longitude)
	}
...
func TestSandbox_MalformedPayloadRejection(t *testing.T) {
	...
	t.Run("BadTokenLookup", func(t *testing.T) {
		_, ok := store.GetNodeByToken("COMPLETELY-FAKE-TOKEN-XYZ")
		if ok {
			t.Error("Expected GetNodeByToken to return false for fake token")
		}
	})
